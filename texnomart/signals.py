...
def post_save_category(sender, instance, created, **kwargs):
    if created:
        ...
        to = 'abdulazizaliyev4542@gmail.com'
...
def pre_delete_category(sender, instance,**kwargs):
    file_path = os.path.join(BASE_DIR,'texnomart/delete_products', f'category.{instance.id}.json')
...
def pre_delete_product(sender, instance,**kwargs):
    file_path = os.path.join(BASE_DIR,'texnomart/delete_products', f'product.{instance.id}.json')
